...
class MillionairesProblem extends Component {
    ...
    async onAddMillionaire({ millionaireAddress, millionaireNetWorth } ) {
        ...
        // )
        const taskFn = 'add_jsondata(string)';
        const taskArgs = [
            [millionaireNetWorth, 'string'],
        ];
        const taskGasLimit = 10000000000000; //should be some large number
        ...
        openSnackbar({ message: 'Task pending: adding dataset to the secret state' });
        ...
        task.ethStatus === 2 ?
            openSnackbar({ message: 'Task succeeded: added dataset' })
            :
    ...
    async onComputeRichest() {
        // Create compute task metadata
        const taskFn = 'return_latest_jsondata()';
        const taskArgs = [];
        const taskGasLimit = 10000000000000;//should be some large number as well
        const taskGasPx = utils.toGrains(1e-7);
        let task = await new Promise((resolve, reject) => {
        ...
        openSnackbar({ message: 'Task pending: running some statistical logic' });
        while (task.ethStatus === 1) {
            task = await this.props.enigma.getTaskRecordStatus(task);
            await sleep(1000);
        }
        if (task.ethStatus === 2) {
            openSnackbar({ message: 'Task succeeded: returned the result' });
            ...
            openSnackbar({ message: 'Task failed: did not return the result' });
        }
    }
        ...
        return (
            <div>
                <Grid container spacing={3}>
                    <Grid item xs={12}>
                        <h3>Computation Test Secret Contract Address: {this.props.deployedMillionairesProblem}</h3>
                    </Grid>
                    <Grid item xs={6}>
                        <div>
                            <Notifier />
                            ...
                            <form>
                                ...
                                <div>
                                    <Field
                                        ...
                                        label="The number of the datset will be the number you enter multiplied by 20"
                                    />
                                </div>
                                <br />
                                <div>
